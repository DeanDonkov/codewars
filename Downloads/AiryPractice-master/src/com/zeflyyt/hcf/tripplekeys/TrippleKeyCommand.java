	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 0 || args.length > 2) {
            sender.sendMessage(
                    "§7§m--------------------------------");
            sender.sendMessage(
            		"§b/tkey start <duration> - Starts the TripleKey timer.");
            sender.sendMessage(
            		"§b/tkey end - Ends the current TripleKey timer.");
            sender.sendMessage(
                    "§7§m--------------------------------");
			return true;
		}
		...
		if (args.length == 2) {
			if (args[0].equalsIgnoreCase("start")) {
				...
				Bukkit.broadcastMessage(ChatColor.GRAY + "The store-wide " + ChatColor.DARK_RED.toString() + ChatColor.BOLD + "TripleKey " + ChatColor.GRAY + "has started at " + ChatColor.DARK_AQUA + ConfigurationService.DONATE_URL

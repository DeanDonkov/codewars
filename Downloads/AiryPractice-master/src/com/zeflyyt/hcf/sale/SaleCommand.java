	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 0 || args.length > 2) {
            sender.sendMessage(
                    "§7§m--------------------------------");
            sender.sendMessage(
            		"§b/sale start <duration> - Starts the sale timer.");
            sender.sendMessage(
            		"§b/sale end - Ends the current sale timer.");
            sender.sendMessage(
                    "§7§m--------------------------------");
			return true;
		}
		...
		if (args.length == 2) {
			if (args[0].equalsIgnoreCase("start")) {
				...
				Bukkit.broadcastMessage(ChatColor.GRAY + "The store-wide " + ChatColor.AQUA.toString() + ChatColor.BOLD + "Sale " + ChatColor.GRAY + "has started at " + ChatColor.DARK_AQUA + ConfigurationService.DONATE_URL

    ...
    public void EntityDamageEvent(EntityDamageByEntityEvent e) {
        if (e.getDamager() instanceof Player && e.getEntity() instanceof Player) {
            ...
            if (this.plugin.getSotwTimer().getSotwRunnable() != null && SotwCommand.enabled.contains(oponent.getUniqueId()) && !SotwCommand.enabled.contains(player.getUniqueId())) {
                ...
                oponent.sendMessage("§7You are not permitted to hit this player, they do not have their §a§lSOTW §7paused.");
            	oponent.sendMessage(ChatColor.GRAY + "§m----------------------------------");
                e.setCancelled(true);
            }
            	...
            	oponent.sendMessage(ChatColor.GRAY + "§m----------------------------------");
            	oponent.sendMessage("§7You cannot hit players whilst sotw is active if you would like to execute §a§l/sotw enable§e.");
            	oponent.sendMessage(ChatColor.GRAY + "§m----------------------------------");
            	e.setCancelled(true);
            }

	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 0 || args.length > 2) {
            sender.sendMessage(
                    "§7§m--------------------------------");
            sender.sendMessage(
            		"§b/keyall start <duration> - Starts the key-all timer.");
            sender.sendMessage(
            		"§b/keyall end - Ends the current keyall timer.");
            sender.sendMessage(
                    "§7§m--------------------------------");
			return true;
		}
		...
		if (args.length == 2) {
			if (args[0].equalsIgnoreCase("start")) {
				...
				Bukkit.broadcastMessage(ChatColor.GRAY + "A KeyAll will " + ChatColor.GRAY + " happen in " + DurationFormatUtils.formatDurationWords(duration, true, true) + ChatColor.GRAY + ".");
			} else {
				sender.sendMessage(ChatColor.RED + "Unknown sub-command!");
				return true;
			}
